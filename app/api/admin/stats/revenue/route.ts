...
import { getRevenueSeries, VALID_SCOPES, VALID_RANGES, type ChannelScope, type StatRange } from "@/lib/stats/channel-breakdown";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const result = await getRevenueSeries({ scope, range });
    return ok(result);
  } catch (e) {
    reportError(e, { route: "GET /api/admin/stats/revenue", tags: { domain: "stats" } });
    console.error("[admin/stats/revenue] GET error", e);
    return Err.internal();
  }
}
