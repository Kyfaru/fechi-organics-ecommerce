...
export async function POST(req: NextRequest) {
  ...
  const { messageId, recipientEmail, subject, threadContent } = JSON.parse(rawBody) as {
    ticketId: string;
    messageId: string;
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
        ...
        <tr>
          <td style="padding:40px 48px;">
            <p style="font-size:15px;color:#40493c;line-height:1.6;white-space:pre-wrap;">${threadContent.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
