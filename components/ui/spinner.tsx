...
export function Spinner({ size = 16, invert, disabled, className, ...props }: SpinnerProps) {
  ...
  return (
    <div className={cn('relative', className)} style={{ width: sizePx, height: sizePx }} {...props}>
      {[...Array(5)].map((_, i) => (
        <div key={i} className="absolute inset-0 flex animate-spin justify-center" style={{ animationDelay: `${i * 100}ms` }}>
          <div style={{ backgroundColor: invert ? 'var(--background)' : 'var(--foreground)', width: barWidth, height: barHeight, borderRadius: '9999px' }} />
        </div>
      ))}
    </div>
  );
